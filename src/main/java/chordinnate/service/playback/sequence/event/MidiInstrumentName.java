...
public class MidiInstrumentName extends MidiEventGenerator {
    ...
    public void addEvent(Sequence sequence, MidiEventDataBundle newEventState) throws InvalidMidiDataException {
        ...
        if (StringUtils.isNotBlank(newEventState.getInstrumentName())) {

            byte[] data = newEventState.getInstrumentName().getBytes(StandardCharsets.US_ASCII);

            MetaMessage mm = new MetaMessage();
            mm.setMessage(0x04, data, data.length);
