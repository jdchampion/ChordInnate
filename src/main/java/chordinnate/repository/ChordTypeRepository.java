...
public interface ChordTypeRepository extends CrudRepository<ChordType, Integer> {
    ...
    List<ChordType> findAllByIntervals(@NotNull @Param("intervals") Collection<Interval[]> intervals);
    ...
    List<ChordType> findAllBySizeRange(@Param("min") int min, @Param("max") int max);

    @Query("SELECT c FROM ChordType c JOIN FETCH ChordTypeTagRelation cttr ON c.id = cttr.matchingChordType.id JOIN FETCH Tag t ON cttr.matchingTag.id = t.id WHERE t.name IN :tags")
    List<ChordType> findAllByTag(@NotNull @Param("tags") Collection<String> tags);
