...
public interface ChordTypeTagRelationRepository extends CrudRepository<ChordTypeTagRelation, Integer> {
    ...
    @Query("SELECT cttr FROM ChordTypeTagRelation cttr JOIN FETCH ChordType st ON st.id = cttr.matchingChordType.id JOIN FETCH ChordTypeTag t ON t.id = cttr.matchingTag.id WHERE cttr.matchingTag = :chordTypeTag")
    List<ChordTypeTagRelation> findAllByTag(@NotNull @Param("chordTypeTag") ChordTypeTag tag);

    @Query("DELETE FROM ChordTypeTagRelation cttr WHERE cttr.matchingTag = :chordTypeTag")
    void deleteAllByTag(@NotNull @Param("chordTypeTag") ChordTypeTag tag);
    ...
    void deleteAllByChordTypeAndTag(@NotNull @Param("chordType") ChordType chordType, @NotNull @Param("chordTypeTag") ChordTypeTag tag);
