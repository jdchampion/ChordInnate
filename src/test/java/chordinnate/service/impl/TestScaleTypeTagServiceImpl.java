...
@PrepareForTest({ScaleTypeTagRepository.class})
public class TestScaleTypeTagServiceImpl {

    @Rule
    public ExpectedException expectedException = ExpectedException.none();

    private static ScaleTypeTagServiceImpl service;
    private static ScaleTypeTagRepository mockRepo;
    private static ScaleTypeTag TEST_TAG;

    @BeforeClass
    public static void init() {
        mockRepo = mock(ScaleTypeTagRepository.class);
        when(mockRepo.save(any(ScaleTypeTag.class))).thenReturn(TEST_TAG);
        doNothing().when(mockRepo).delete(any(ScaleTypeTag.class));

        service = new ScaleTypeTagServiceImpl(mockRepo);
    }

    @Before
    public void setup() {
        TEST_TAG = new ScaleTypeTag();
    ...
    public void delete_blankParam() {
        service.delete(new ScaleTypeTag());
    }
    ...
    public void delete_existingRecordDoesNotMatchEntity() {
        ...
        ScaleTypeTag inRepo = new ScaleTypeTag();
        inRepo.setId(1);
